...
const ChatGPT = ()=>{
  ...
  console.log(recording)
    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor:themes.backgroundColor}}>
       
        <div style={{ backgroundColor: themes.backgroundColor }} className={`w-full h-18  fixed  justify-between`}>
        ...
        <div className='py-32'>
          <div className="chatsBox max-w-2xl mx-auto space-y-12 grid grid-cols-1 scroll-smooth scrollbar-hide overflow-x-hidden" style={{ maxHeight: '30rem' }}>
            {loading && 
        ...
        <div className={`w-full fixed bottom-0`}>
          <div className='justify-end items-center bg-white rounded-xl flex mx-96 my-3'>
            ...
            />
            <button className='bg-white btn px-4' disabled={!browserSupportsSpeechRecognition} onClick={handleRecording}>
              {recording ? <i class="fa-solid fa-microphone w-10"></i> : <i class="fa-solid fa-microphone-slash w-10"></i>}
            </button>
            <button  className={`p-4 rounded-r-xl btn`} onClick={sendMessage}>
            <i class="fa-sharp fa-solid fa-arrow-up-right-from-square w-8"></i>
            </button>
          </div>
        </div>
      </div>
